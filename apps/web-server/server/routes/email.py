...
def _get_oauth_redirect_uri(request: Request) -> str:
    ...
    Uses EMAIL_OAUTH_REDIRECT_URI env var if set, otherwise derives from request.
    The env var is useful when the app is behind a reverse proxy or accessed via
    a LAN IP, since Azure requires HTTPS for non-localhost redirect URIs.
    """
    override = os.environ.get("EMAIL_OAUTH_REDIRECT_URI")
    if override:
        return override.rstrip("/")
    return str(request.base_url).rstrip("/") + "/api/email/auth/outlook/callback"
