...
async def get_worktree_diff(task_id: str):
    ...
    if isinstance(projects_data, dict):
        ...
        for project in projects_data:
            path = Path(project.get("path", ""))
            if project_id and project.get("id")API Error: Claude's response exceeded the 32000 output token maximum. To configure this behavior, set the CLAUDE_CODE_MAX_OUTPUT_TOKENS environment variable.