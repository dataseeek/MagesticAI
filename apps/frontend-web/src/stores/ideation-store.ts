...
export async function loadIdeation(projectId: string): Promise<void> {
  ...
  const result = await window.API.getIdeation(projectId);
...
export function generateIdeation(projectId: string): void {
  ...
  window.API.generateIdeation(projectId, config);
}
...
export async function stopIdeation(projectId: string): Promise<boolean> {
  ...
  const result = await window.API.stopIdeation(projectId);
...
export async function refreshIdeation(projectId: string): Promise<void> {
  ...
  await window.API.stopIdeation(projectId);
  ...
  window.API.refreshIdeation(projectId, config);
}

export async function dismissAllIdeasForProject(projectId: string): Promise<boolean> {
  const store = useIdeationStore.getState();
  const result = await window.API.dismissAllIdeas(projectId);
  if (result.success) {
...
export async function archiveIdeaForProject(projectId: string, ideaId: string): Promise<boolean> {
  const store = useIdeationStore.getState();
  const result = await window.API.archiveIdea(projectId, ideaId);
  if (result.success) {
...
export async function deleteIdeaForProject(projectId: string, ideaId: string): Promise<boolean> {
  const store = useIdeationStore.getState();
  const result = await window.API.deleteIdea(projectId, ideaId);
  if (result.success) {
...
export async function deleteMultipleIdeasForProject(projectId: string, ideaIds: string[]): Promise<boolean> {
  const store = useIdeationStore.getState();
  const result = await window.API.deleteMultipleIdeas(projectId, ideaIds);
  if (result.success) {
...
export function appendIdeation(projectId: string, typesToAdd: IdeationType[]): void {
  ...
  window.API.generateIdeation(projectId, appendConfig);
}
...
export function setupIdeationListeners(): () => void {
  ...
  const unsubProgress = window.API.onIdeationProgress((projectId, status) => {
  ...
  const unsubLog = window.API.onIdeationLog((projectId, log) => {
    if (!isCurrentProject(projectId)) return;
    store().addLog(log);
  });

  // Listen for individual ideation type completion (streaming)
  const unsubTypeComplete = window.API.onIdeationTypeComplete(
  ...
  const unsubTypeFailed = window.API.onIdeationTypeFailed(
  ...
  const unsubComplete = window.API.onIdeationComplete((projectId, session) => {
    // Only process events for the current project
    if (!isCurrentProject(projectId)) {
      if (window.DEBUG) {
  ...
  const unsubError = window.API.onIdeationError((projectId, error) => {
    // Only process events for the current project
    if (!isCurrentProject(projectId)) return;
  ...
  const unsubStopped = window.API.onIdeationStopped((projectId) => {
    // Only process events for the current project
    if (!isCurrentProject(projectId)) return;
