...
export async function loadRoadmap(projectId: string): Promise<void> {
  ...
  const statusResult = await window.API.getRoadmapStatus(projectId);
  ...
  const result = await window.API.getRoadmap(projectId);
  if (result.success && result.data) {
    ...
    if (migratedRoadmap !== result.data) {
      window.API.saveRoadmap(projectId, migratedRoadmap).catch((err) => {
...
export function generateRoadmap(
  ...
  window.API.generateRoadmap(projectId, enableCompetitorAnalysis, refreshCompetitorAnalysis);
}
...
export function refreshRoadmap(
  ...
  window.API.refreshRoadmap(projectId, enableCompetitorAnalysis, refreshCompetitorAnalysis);
}
...
export async function stopRoadmap(projectId: string): Promise<boolean> {
  ...
  const result = await window.API.stopRoadmap(projectId);
