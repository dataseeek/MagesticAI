...
export function initializeMRReviewListeners(): void {
  ...
  if (!window.API?.onGitLabMRReviewProgress) {
  ...
  window.API.onGitLabMRReviewProgress(progressHandler);
  ...
  window.API.onGitLabMRReviewComplete(completeHandler);
  ...
  window.API.onGitLabMRReviewError(errorHandler);

  // Store cleanup functions if the API supports removeListener
  // Note: These are optional methods that may not exist in the API
...
export function startMRReview(projectId: string, mrIid: number): void {
  ...
  window.API.runGitLabMRReview(projectId, mrIid);
}
...
export function startFollowupReview(projectId: string, mrIid: number): void {
  const store = useMRReviewStore.getState();
  store.startMRReview(projectId, mrIid);
  window.API.runGitLabMRFollowupReview(projectId, mrIid);
}
