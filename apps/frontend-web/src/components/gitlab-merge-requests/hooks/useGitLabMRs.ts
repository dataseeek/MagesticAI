...
export function useGitLabMRs(projectId?: string): UseGitLabMRsResult {
  ...
  const fetchMRs = useCallback(async () => {
    ...
    try {
      ...
      const connectionResult = await window.API.checkGitLabConnection(projectId);
      if (connectionResult.success && connectionResult.data) {
        ...
        if (connectionResult.data.connected) {
          // Fetch MRs
          const result = await window.API.getGitLabMergeRequests(projectId, stateFilter);
          if (result.success && result.data) {
            ...
            result.data.forEach(mr => {
              ...
              if (!existingState?.result && window.API.getGitLabMRReview) {
                window.API.getGitLabMRReview(projectId, mr.iid).then(reviewResult => {
                  if (reviewResult) {
  ...
  const selectMR = useCallback((mrIid: number | null) => {
    ...
    if (mrIid && projectId) {
      ...
      // Only fetch from disk if we don't have a result in the store
      if (!existingState?.result && window.API.getGitLabMRReview) {
        window.API.getGitLabMRReview(projectId, mrIid).then(result => {
          if (result) {
            // Update store with the loaded result
  ...
  const checkNewCommits = useCallback(async (mrIid: number): Promise<GitLabNewCommitsCheck> => {
    if (!projectId || !window.API.checkGitLabMRNewCommits) {
      return { hasNewCommits: false };
    }

    try {
      const result = await window.API.checkGitLabMRNewCommits(projectId, mrIid);
  ...
  const cancelReview = useCallback(async (mrIid: number): Promise<boolean> => {
    if (!projectId || !window.API.cancelGitLabMRReview) return false;

    try {
      const success = await window.API.cancelGitLabMRReview(projectId, mrIid);
      if (success) {
  ...
  const postReview = useCallback(async (mrIid: number, selectedFindingIds?: string[]): Promise<boolean> => {
    if (!projectId || !window.API.postGitLabMRReview) return false;

    try {
      return await window.API.postGitLabMRReview(projectId, mrIid, selectedFindingIds);
    } catch (err) {
  ...
  const postNote = useCallback(async (mrIid: number, body: string): Promise<boolean> => {
    if (!projectId || !window.API.postGitLabMRNote) return false;

    try {
      return await window.API.postGitLabMRNote(projectId, mrIid, body);
    } catch (err) {
  ...
  const mergeMR = useCallback(async (mrIid: number, mergeMethod: 'merge' | 'squash' | 'rebase' = 'squash'): Promise<boolean> => {
    if (!projectId || !window.API.mergeGitLabMR) return false;

    try {
      const success = await window.API.mergeGitLabMR(projectId, mrIid, mergeMethod);
      if (success) {
  ...
  const assignMR = useCallback(async (mrIid: number, userIds: number[]): Promise<boolean> => {
    if (!projectId || !window.API.assignGitLabMR) return false;

    try {
      const success = await window.API.assignGitLabMR(projectId, mrIid, userIds);
      if (success) {
  ...
  const approveMR = useCallback(async (mrIid: number): Promise<boolean> => {
    if (!projectId || !window.API.approveGitLabMR) return false;

    try {
      return await window.API.approveGitLabMR(projectId, mrIid);
    } catch (err) {
