...
export function GitLabIntegration({
  ...
  useEffect(() => {
    const checkGlab = async () => {
      ...
      try {
        const result = await window.API.checkGitLabCli();
  ...
  const fetchUserProjects = async () => {
    ...
    try {
      ...
      const result = await window.API.listGitLabUserProjects(hostname);
  ...
  const handleOAuthSuccess = async () => {
    ...
    try {
      const hostname = envConfig?.gitlabInstanceUrl?.replace(/^https?:\/\//, '').replace(/\/$/, '');
      const tokenResult = await window.API.getGitLabToken(hostname);
      if (tokenResult.success && tokenResult.data?.token) {
        updateEnvConfig({ gitlabToken: tokenResult.data.token });
      }

      const userResult = await window.API.getGitLabUser(hostname);
  ...
  const handleStartOAuth = async () => {
    const hostname = envConfig?.gitlabInstanceUrl?.replace(/^https?:\/\//, '').replace(/\/$/, '');
    const result = await window.API.startGitLabAuth(hostname);

    if (result.success) {
      // Poll for auth completion
      const checkAuth = async () => {
        const authResult = await window.API.checkGitLabAuth(hostname);
  ...
  const handleInstallGlab = async () => {
    ...
    try {
      const result = await window.API.installGitLabCli();
